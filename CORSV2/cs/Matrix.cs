...
namespace CORSV2
{
    class Matrix
    {
        ...
        public Matrix(double p, int p_2)
        {
        ...
        {
            Matrix tm = new Matrix(m.Rows, m.Columns);
        ...
        ////////////////重载/矩阵主对角线元素除以一个double的实数
    
        public static Matrix operator /(double t, Matrix m)
        {
        ...
        /// 判断矩阵是否属于同类型矩阵
        /// </summary>
        /// <param name="M">对比矩阵</param>
        /// <returns> bool 型，矩阵是否同类型</returns>
        public bool IsSimilar(Matrix M)
        {
        ...
        /// 判断矩阵是否为零矩阵
        /// </summary>
        /// <returns>bool 型，矩阵是否为零矩阵</returns>
        public bool IsZeroMatrix()
        {
            foreach (double e in this.elements)
            {
                if (e > 1e-7)
                    return false;
            }
        ...
        /// 增广矩阵的列，在矩阵右边放置一个矩阵
        /// </summary>
        /// <param name="M">需要放置的矩阵</param>
        /// <returns>增广炬阵</returns>
        public Matrix columAugment(Matrix M)
        {
            Trace.Assert(this.Rows == M.Rows, "矩阵的行数不相同");
            Matrix res = new Matrix(this.Rows, this.Columns + M.Columns);
        ...
        /// <summary>
        /// 增广炬阵的行，在矩阵的的下方放置一个矩阵
        /// </summary>
        /// <param name="M">需要放置的矩阵</param>
        /// <returns>增广炬阵</returns>
        public Matrix rowAugment(Matrix M)
        {
            Trace.Assert(this.Columns == M.Columns, "矩阵的列数不相同");
            Matrix res = new Matrix(this.Rows + M.Rows, this.Columns);
