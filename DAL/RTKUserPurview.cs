...
namespace DAL
{
    ...
    public class RTKUserPurview
    {
        ...
        public static bool Update1(Model.RTKUserPurview model)
        {
            string strSql = "update RTKUserPurview set  EndTime=@EndTime  where  UserName= '" + model.UserName + "'";

            SqlParameter EndTime = new SqlParameter("EndTime", SqlDbType.DateTime); EndTime.Value = model.EndTime;

            return DBHelperSQL.GetNums(strSql, new SqlParameter[] { EndTime }, connectionString) == 1 ? true : false;
        }
