...
namespace DAL
{
    public class RTKSurveyRec
    {
        ...
        public static System.Data.DataSet GetList1(string username)
        {
            string strSql = "select * from RTKSurveyRec where UserName='"+username+"'";
        ...
        public static DataSet GetListByPage1(int offset, int limit,string username, string sort = "StartTime", string order = "desc", string search = "")
        {
            string deorder = order.ToLower() == "desc" ? "asc" : "desc";
            int endRecord = offset + limit;
            string sql = "SELECT * FROM RTKSurveyRec w1,( SELECT TOP @limit w.ID FROM( SELECT TOP  @endRecord * FROM RTKSurveyRec where @search ORDER BY @sort @order) w ORDER BY w.@sort @deorder) w2 WHERE w1.ID = w2.ID and UserName='"+username+"'  ORDER BY w1.@sort @order";
            sql = sql.Replace("@limit", limit.ToString());
            sql = sql.Replace("@sort", sort);
            sql = sql.Replace("@endRecord", endRecord.ToString());
            sql = sql.Replace("@order", order);
