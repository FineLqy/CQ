...
namespace DAL
{
    ...
    public class InvoiceList
    {
        ...
        public static bool Add(Model.InvoiceList model)
        {
            ...
            return DBHelperSQL.GetNums(strSql, new SqlParameter[] { UserName, OrderNumber, OrderDetail, Price, SubmitTime, DealTime, PayTime, Status, InvoiceInfoIndex, InvoiceType, IssueType, InvoiceFile, ContractFile, ContractStatus,BelongArea,Type }, connectionString) == 1 ? true : false;
        }
        ...
        public static Model.InvoiceList GetModel(string OrderNumber)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                model.ContractStatus = Convert.ToInt32(ds.Tables[0].Rows[0]["ContractStatus"]);
        ...
        /// <returns></returns>
        public static DataSet GetBriefList2(int offset, int limit, string sort = "SubmitTime", string order = "desc", string search = "", string strwhere = "1=1")
        {
            string deorder = order.ToLower() == "desc" ? "asc" : "desc";
            int endRecord = offset + limit;
            string sql = "SELECT * FROM InvoiceList w1,( SELECT TOP " + limit + " w.ID FROM( SELECT TOP  " + endRecord + "* FROM InvoiceList where OrderNumber like '%" + search + "%' and " + strwhere + " ORDER BY @sort @order and ContractStatus=4) w ORDER BY w.@sort @deorder) w2 WHERE w1.ID = w2.ID ORDER BY w1.@sort @order";
            sql = sql.Replace("@sort", sort);
            sql = sql.Replace("@order", order);
            sql = sql.Replace("@deorder", deorder);
            return DBHelperSQL.GetDataSet(sql, connectionString);
        ...
        {
            string strSql = "select count(*) from InvoiceList where Status=2";
            return Convert.ToInt32(DBHelperSQL.GetResult(strSql, connectionString));
        }
