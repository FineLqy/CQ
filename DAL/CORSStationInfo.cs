...
namespace DAL
{
    ...
    public class CORSStationInfo
    {
        ...
        {
            string strSql = "insert into CORSStationInfo(StationName,StationOName, IP, Port, Lat, Lon,H, IsOK,TransferType,ReceiverType,ReceiverSN,AntennaType,AntennaSN,StationFigure,StationType,SpecialTelLine,ContactPerson,ContactTel,FaultLog,TrusteeFeeLog,OperatingState,Condition,MovingStationLog,AntennaRepLog, Remark,BuildTime,CaseNumber,AffiliatedNetwork,PiersType,RelyUnits,Address,ThicknessOfLayer,TrafficCondition,SitePerson,Builder,SoilType,PermafrostDepth,ThawDepth,GroundwaterDepth,BelongsMap,GeologicalStructure,StationPlan,RingView,EnvironmentalDescription,MaintenanceUnit,SiteConditions, GravityPier, LevelSign, LightningReport, StationPhoto,SatNum ) values(@StationName,@StationOName, @IP, @Port, @Lat, @Lon,@H, @IsOK,@TransferType, @ReceiverType, @ReceiverSN, @AntennaType, @AntennaSN, @StationFigure, @StationType, @SpecialTelLine, @ContactPerson, @ContactTel, @FaultLog, @TrusteeFeeLog, @OperatingState, @Condition, @MovingStationLog, @AntennaRepLog, @Remark,@BuildTime,@CaseNumber,@AffiliatedNetwork,@PiersType,@RelyUnits,@Address,@ThicknessOfLayer,@TrafficCondition,@SitePerson,@Builder,@SoilType,@PermafrostDepth,@ThawDepth,@GroundwaterDepth,@BelongsMap,@GeologicalStructure,@StationPlan,@RingView,@EnvironmentalDescription,@MaintenanceUnit,@SiteConditions,@GravityPier, @LevelSign, @LightningReport, @StationPhoto,@SatNum)";
            ...
            SqlParameter SatNum = new SqlParameter("SatNum", SqlDbType.NVarChar); SatNum.Value = model.SatNum.ToString();
            



            return DBHelperSQL.GetNums(strSql, new SqlParameter[] { StationName, StationOName, IP, Port, Lat, Lon, H, IsOK, TransferType, ReceiverType, ReceiverSN, AntennaType, AntennaSN, StationFigure, StationType, SpecialTelLine, ContactPerson, ContactTel, FaultLog, TrusteeFeeLog, OperatingState, Condition, MovingStationLog, AntennaRepLog, Remark, BuildTime, CaseNumber, AffiliatedNetwork, PiersType, RelyUnits, Address, ThicknessOfLayer, TrafficCondition, SitePerson, Builder, SoilType, PermafrostDepth, ThawDepth, GroundwaterDepth, BelongsMap, GeologicalStructure, StationPlan, RingView, EnvironmentalDescription, MaintenanceUnit, SiteConditions, GravityPier, LevelSign, LightningReport, StationPhoto, SatNum }, connectionString) == 1 ? true : false;
        }
        ...
        {
            string strSql = "update CORSStationInfo set StationName=@StationName, StationOName=@StationOName, IP=@IP, Port=@Port, Lat=@Lat, Lon=@Lon, H=@H, IsOK=@IsOK, TransferType=@TransferType, ReceiverType=@ReceiverType, ReceiverSN=@ReceiverSN, AntennaType=@AntennaType, AntennaSN=@AntennaSN, StationFigure=@StationFigure, StationType=@StationType, SpecialTelLine=@SpecialTelLine, ContactPerson=@ContactPerson, ContactTel=@ContactTel, FaultLog=@FaultLog, TrusteeFeeLog=@TrusteeFeeLog, OperatingState=@OperatingState, Condition=@Condition, MovingStationLog=@MovingStationLog, AntennaRepLog=@AntennaRepLog, Remark=@Remark,BuildTime=@BuildTime,CaseNumber=@CaseNumber,AffiliatedNetwork=@AffiliatedNetwork,PiersType=@PiersType,RelyUnits=@RelyUnits,Address=@Address,ThicknessOfLayer=@ThicknessOfLayer,TrafficCondition=@TrafficCondition,SitePerson=@SitePerson,Builder=@Builder,SoilType=@SoilType,PermafrostDepth=@PermafrostDepth,ThawDepth=@ThawDepth,GroundwaterDepth=@GroundwaterDepth,BelongsMap=@BelongsMap,GeologicalStructure=@GeologicalStructure,StationPlan=@StationPlan,RingView=@RingView,EnvironmentalDescription=@EnvironmentalDescription,MaintenanceUnit=@MaintenanceUnit,SiteConditions=@SiteConditions, GravityPier=@GravityPier, LevelSign=@LevelSign, LightningReport=@LightningReport, StationPhoto=@StationPhoto,SatNum=@SatNum where StationName = '" + model.StationName + "'";
            SqlParameter StationName = new SqlParameter("StationName", SqlDbType.NVarChar); StationName.Value = model.StationName;
            SqlParameter StationOName = new SqlParameter("StationOName", SqlDbType.NVarChar); StationOName.Value = model.StationOName;
            SqlParameter IP = new SqlParameter("IP", SqlDbType.NVarChar); IP.Value = model.IP;
            SqlParameter Port = new SqlParameter("Port", SqlDbType.NVarChar); Port.Value = model.Port;
        ...
        public static Model.CORSStationInfo GetModel(string StationName)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                model.SatNum= Convert.ToInt32(ds.Tables[0].Rows[0]["SatNum"]);
                return model;
            }
            else
            {
